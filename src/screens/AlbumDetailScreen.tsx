...
export default function AlbumDetailScreen({ route, navigation }: any) {
  ...
  useEffect(() => { (async () => { try { const [al, td] = await Promise.all([deezerApi.getAlbum(id), deezerApi.getAlbumTracks(id)]); setCover(al.cover_big || al.cover_medium || ''); setArtist(al.artist?.name || ''); setTracks(td.map((t: any) => ({ id: t.id, title: t.title, artist: t.artist.name, artistId: t.artist.id, album: al.title, albumId: al.id, duration: t.duration, preview: t.preview, cover: al.cover_medium, coverBig: al.cover_big }))); } catch (e) { console.log(e); } setLoading(false); })(); }, [id]);
  ...
  return (
    <View style={s.c}><ScrollView showsVerticalScrollIndicator={false}>
      ...
      <View style={s.tl}>{tracks.map((tr, i) => (
        <TouchableOpacity key={tr.id} style={s.tr} onPress={() => play(tr, tracks)}>
          <Text style={s.tn}>{i + 1}</Text>
          <View style={s.ti}><Text style={[s.tt, currentTrack?.id === tr.id && { color: COLORS.primary }]} numberOfLines={1}>{tr.title}</Text><Text style={s.ta} numberOfLines={1}>{tr.artist}</Text></View>
        </TouchableOpacity>
      ))}</View>
...
const s = StyleSheet.create({
  ...
  cv:{width:width*0.55,height:width*0.55,borderRadius:8,marginTop:SPACING.xl},t:{fontSize:22,fontWeight:'bold',color:COLORS.textPrimary,marginTop:SPACING.lg,textAlign:'center'},ar:{fontSize:15,color:COLORS.textSecondary,marginTop:SPACING.xs},
  ...
  tn:{fontSize:15,color:COLORS.textSecondary,width:24,textAlign:'center'},ti:{flex:1},tt:{fontSize:15,fontWeight:'500',color:COLORS.textPrimary},ta:{fontSize:13,color:COLORS.textSecondary,marginTop:2},
});
